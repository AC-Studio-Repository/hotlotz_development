...
class AuctionApproveListener
{
     ...
     * Handle the event.
     *
     * @param  AuctionApproveEvent  $event
     * @return void
     */
    public function handle(AuctionApproveEvent $event)
    {
        \Log::channel('getAuctionsStatusLog')->info('Start - AuctionApproveEvent');
        ...
        try {
            ...
            if($auction != null && $auction->sr_auction_id != null){
                ...
                if ( isset($result) && !isset($result['error'])) {
                    \Log::channel('getAuctionsStatusLog')->info('Auction is approved : '.$result['is_approved']);
                    $auction_data = [];
                    if ($result['is_approved']) {
                        $auction_data['status'] = 'Approved';
        ...
        \Log::channel('getAuctionsStatusLog')->info('End - AuctionApproveEvent');
    }
}
