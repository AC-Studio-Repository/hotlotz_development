...
<div class="card card-accent-secondary">
    ...
    {!! Form::model($blogs, ['route' => ['auction_cms.auction_cmss.updateContent'], 'method' => 'POST']) !!}
            ...
            <input type="hidden" id="hid_backend_count" name="hid_backend_count" value="{{ $latest_id }}" />
                ...
                <div class="row">
                    <div class="form-group col-12 col-md-12 col-xl-12">
                        ...
                        <div class="file-loading">
                            <input id="auctioncms_banner_image" name="auctioncms_banner_image" type="file" accept="image/*">
                        </div>

                        @if(!$auction_cms_data->isEmpty())
                            <input type="text" style="display: none;" name="hide_auctioncms_image_ids" id="hide_auctioncms_image_ids" value="{{  $auction_cms->banner_image }}" data-parsley-required="false" data-parsley-errors-container="#error_uploaded_item_images_block" data-parsley-required-message="Please select and upload banner image!">
                        @else
                            <input type="text" style="display: none;" name="hide_auctioncms_image_ids" id="hide_auctioncms_image_ids" value="" data-parsley-required="false" data-parsley-errors-container="#error_uploaded_item_images_block" data-parsley-required-message="Please select and upload banner image!">
                        @endif
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            {{ Form::text('caption', (!$auction_cms_data->isEmpty()) ? $auction_cms->caption : null, [
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            {{ Form::text('blog_header_1', (!$auction_cms_data->isEmpty()) ? $auction_cms->blog_header_1 : null, [
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        @if(!$auction_cms_data->isEmpty())
                            <textarea id="title_blog" name="title_blog" class="summernote">{!! $auction_cms->blog_1 !!}</textarea>
                        @else
        ...
        <div class="card-footer">
            <!-- <button class="btn btn-success" onclick="edit_content();">{{ __('Update Auction') }}</button> -->
            <button type="submit" class="btn btn-success">{{ __('Update Auction') }}</button>
...
<script>
 ...
 var blog_count = {{ $blog_count }};
    ...
    $(document).ready(function() {
        ...
        $("#auctioncms_banner_image").fileinput({
            theme: "fas",
            uploadUrl: '/manage/auction_cmss/banner_image_upload',
            uploadAsync: true,
            ...
            if(data.response.ids){
                ...
                var hide_item_ids_val = $("#hide_auctioncms_image_ids").val();
                hide_item_ids_val += data.response.ids[0] + ',';
                $('#hide_auctioncms_image_ids').val(data.response.saved_filepath);
            }
        ...
        .on("filedeleted", function(event,key,data) {
            ...
            var hide_item_ids = $("#hide_auctioncms_image_ids").val();
            hide_item_ids = hide_item_ids.replace((item_img_id + ','),'');
            $('#hide_auctioncms_image_ids').val(hide_item_ids);
    ...
    function edit_content(){
        ...
        var banner_image = $('#hide_auctioncms_image_ids').val();
        var caption = $('#caption').val();
        var blog_header_1 = $('#blog_header_1').val();
        var blog_header_2 = $('#blog_header_2').val();
        var blog_header_3 = $('#blog_header_3').val();
        ...
        if (!$('#blog_three').summernote('isEmpty')){
            blog_3 = JSON.stringify($('.summernote').eq(2).summernote('code'));
        }

        $.ajax({
            url: "/manage/auction_cmss/updateContent",
            type: 'post',
            data:{banner_image, caption, blog_header_1, blog_1, blog_header_2, blog_2, blog_header_3, blog_3, _token: _token },
            dataType: 'json',
            async: false,
            success: function(response) {
                if(response.status == '1'){
                    window.location.href = "{{ route('auction_cms.auction_cmss.index')}}";
                }
            }
        });
    }
