    ...
    <div class="card card-accent-secondary">
        ...
        <div class="card-header">
            ...
            <div class="card-actionbar">
                @can('create HowToBuy')
                <a href="{{ route('business_seller.business_sellers.create') }}" class="btn btn-sm btn-outline-success float-right">
                    <i class="zmdi zmdi-plus"></i>
                    {{ __('Display Business Seller') }}
                </a>
                @endcan
            </div>
                ...
                <div class="row">
                    <div class="form-group col-12 col-md-12 col-xl-12">
                        <label class="form-control-label">{{ __('Banner Image') }}</label>
                         @if($banner != '')
                            <div id="old_image">
                                <img onclick="imagepreview(this)" lazyload="on" src="{{
                ...
                <div class="row">
                    <div class="form-group col-12 col-md-12 col-xl-12">
                        <label class="form-control-label">{{ __('Title Blog') }}</label>
                         @if(!$business_seller_data->isEmpty())
                            <div>
                                <div>{!! $business_seller->blog_1 !!}</div>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="card-footer">
            <a href="{{route('business_seller.business_sellers.editcontent')}}"><button class="btn btn-success">{{ __('Edit') }}</button></a>
            <a href="#" onclick="history.back()" class="btn btn-link text-muted">{{ __('Cancel') }}</a>
            <a href="{{ route('services.business-seller') }}" target="_blank" class="btn btn-link text-muted">{{ __('Link to Frontend') }}</a>
        </div>
    </div>
@stop