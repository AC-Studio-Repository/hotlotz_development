...
@section('content')
    ...
    <div class="card card-accent-secondary">
        ...
        <div class="card-block">
            @include('marketplace::all_items_filter')
        </div>

        <div class="card-block" id="divItemList">
            @include('marketplace::all_items_table')
        </div>
...
<script type="text/javascript">
    ...
    var item_list = [];
    var mp_selected_all = {!! json_encode($mp_selected_all) !!};

    $(function(){

        checkNewAdditionSelectAll(mp_selected_all);
        ...
        $(document).on('click', '#mp_all', function(event){
            // console.log('mp_all ', $(this));
            mp_selected_all = 'N';
            if($(this).is(":checked")){
                mp_selected_all = 'Y';
            }
    ...
    {
        var url = "/manage/marketplaces/marketplace_all_filter";
        if(page != null){
            url = "/manage/marketplaces/marketplace_all_filter?page="+page;
        }
        $.ajax({
            url:url,
            type: 'post',
    ...
    function checkNewAdditionSelectAll(mp_selected_all)
    {
        // console.log('mp_selected_all ', mp_selected_all);
        var item_list = [];
        if(mp_selected_all == 'Y'){
            $('#mp_all').prop('checked', true);
            $('.item_id').each(function(index){
                $(this).prop('checked', true);
                item_list.push( $(this).val() );
            });
        }
        if(mp_selected_all == 'N'){
