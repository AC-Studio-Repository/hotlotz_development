...
@section('content')
    ...
    <div class="card card-accent-secondary">
        ...
        {!! Form::model($main_banners, ['route' => ['marketplace_home_banner.marketplace_home_banners.storeInfo'], 'method' => 'POST']) !!}
        <div class="card-block">
            ...
            @if($status == 'create')
                ...
                <div class="row" id="toggle_div_{{ $banner_count }}">
                    <div class="col-md-4">
                        <label>{{ __('Learn More') }}</label>
                        <div class="form-group">
                            {{ Form::text('learn_more_'.$banner_count, null, [
                                    'class' => 'form-control form-control-md' . ($errors->has('learn_more_'.$banner_count) ? ' is-invalid' : ''),
                                    'placeholder' => __('Learn More')
                                ])
                            }}

                            @if ($errors->has('learn_more_'.$banner_count))
                                <div class="invalid-feedback">{{ $errors->first('learn_more_'.$banner_count) }}</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                @foreach($main_banners as $banner)
                    ...
                    <div class="row" id="toggle_learnmore_{{ $banner->id }}">
                        <div class="col-md-4">
                            <label>{{ __('Learn More') }}</label>
                            <div class="form-group">
                                {{ Form::text('learn_more_'.$banner->id, $banner->learn_more, [
                                        'class' => 'form-control form-control-md' . ($errors->has('learn_more_'.$banner->id) ? ' is-invalid' : ''),
                                        'placeholder' => __('Learn More')
                                    ])
                                }}

                                @if ($errors->has('learn_more_'.$banner->id))
                                    <div class="invalid-feedback">{{ $errors->first('learn_more_'.$banner->id) }}</div>
                                @endif
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function initialize_file_upload(banner_count, latest_id, previewImage)
    {
        ...
        $("#banner_"+latest_id).fileinput({
            theme: "fas",
            uploadUrl: '/manage/marketplace_home_banners/marketplace_home_banner_upload',
            uploadAsync: true,
    ...
    function add_new_banner()
    {
        ...
        append_html = '<div class="row" id="toggle_caption_'+latest_id+'">'+
                '<div class="col-md-4">'+
                    "<label>{{ __('Caption') }}</label>"+
                    '<div class="form-group">'+
                        '<input type="text" id="caption_'+latest_id+'" name="caption_'+latest_id+'" value="" class="form-control form-control-md" placeholder="Caption" />'+
                    '</div>'+
                '</div>'+
            '</div>';
        ...
        append_html += '<div class="row" id="toggle_learnmore_'+latest_id+'">'+
                '<div class="col-md-4">'+
                    "<label>{{ __('Learn More') }}</label>"+
                    '<div class="form-group">'+
                        '<input type="text" id="learn_more_'+latest_id+'" name="learn_more_'+latest_id+'" value="" class="form-control form-control-md" placeholder="Learn More" />'+
                    '</div>'+
                '</div>'+
            '</div>';
