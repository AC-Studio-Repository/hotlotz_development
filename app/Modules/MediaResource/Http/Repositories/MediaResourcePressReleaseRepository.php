...
class MediaResourcePressReleaseRepository
{
    public function __construct(MediaResourcePressRelease $pressRelease) {
    ...
    public function create($payload) {
        return $this->pressRelease->create($payload);
    }
    ...
    public function canDestroy($id) {
        return $this->pressRelease->where('id', $id)->doesntHave('childrens')->exists();
    }

    public function destroy($id, $type = 1) { // 1 - normal, 2 - force
        if ($type == 1) {
            return $this->pressRelease->destroy($id);
        } else {
    ...
    public function restore($id) {
        return $this->pressRelease->withTrashed()->find($id)->restore();
    }
}