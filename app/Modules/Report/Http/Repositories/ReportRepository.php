...
namespace App\Modules\Report\Http\Repositories;
...
class ReportRepository
{
    ...
    public function create($payload) {
        return $this->report->create($payload);
    }
    ...
    public function canDestroy($id) {
        return $this->report->where('id', $id)->doesntHave('childrens')->exists();
    }

    public function destroy($id, $type = 1) { // 1 - normal, 2 - force
        if ($type == 1) {
            return $this->report->destroy($id);
        } else {
            return $this->report->withTrashed()->find($id)->forceDelete();
        }
    }

    public function restore($id) {
        return $this->report->withTrashed()->find($id)->restore();
    }
}